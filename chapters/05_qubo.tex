...
\begin{definition}[QUBO~\cite{glover2022qubo}]
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Квантна еквиваленција:} QUBO директно одговара
        ...
        квантних анилера и QAOA алгоритма~\cite{mcgeoch2014}.
\end{itemize}
...
$\lambda > 0$ (довољно великим да казни свако кршење услова)~\cite{glover2022qubo}:
...
Вратимо се проблему \textsc{MaxCut} из поглавља~\ref{sec:maxcut}~\cite{glover2022qubo}.
...
икакве измене формулације проблема~\cite{mcgeoch2014}.
\end{remark}
...
\begin{lstlisting}[style=python, caption={Решавање QUBO помоћу Z3 Optimize пакета.}]
from z3 import *

# QUBO matrica za MaxCut na K4 (simetricna forma)
Q = {
...
if opt.check() == sat:
    ...
    print(f"Resenje x: {vals}")                   # [1, 1, 0, 0]
\end{lstlisting}
\end{example}
...
\begin{example}[Реконструкција  могућих $Q$ за коју је $(0,0,1,1)$ оптимум (Python)]
...
# Slobodne (cjelobrojne) promenljive za simetricnu Q matricu
Q = {(i, j): Int(f'Q_{i}_{j}')
     for i in range(n) for j in range(n)}

s = Solver()

# Simetricnost: Q[i,j] == Q[j,i]
for i in range(n):
    for j in range(n):
        s.add(Q[(i, j)] == Q[(j, i)])

# Ogranicenje domene da pretraga bude konacna
for i in range(n):
    for j in range(i, n):
        s.add(Q[(i, j)] >= -10)
        s.add(Q[(i, j)] <=  10)
...
# Enumeracija prvih 100 razlicitih simetricnih Q matrica
count = 0
while count < 100 and s.check() == sat:
    m  = s.model()
    count += 1
    vals = {(i, j): m.eval(Q[(i, j)]).as_long()
            for i in range(n) for j in range(i, n)}
    print(f"\nMatrica {count}:")
    for i in range(n):
        print([vals.get((min(i,j), max(i,j))) for j in range(n)])
    # Blokiranje trenutnog resenja
    s.add(Or([Q[(i, j)] != vals[(i, j)]
              for i in range(n) for j in range(i, n)]))

print(f"\nUkupno pronadjeno: {count} matrica")
\end{lstlisting}
\end{example}

З3 проналази могућа решења под датим условима ограничења. Свака пронађена матрица $Q$
задовољава услов оптималности за $\mathbf{x}^*$, али може имати различите вредности.
Тиме се добијају QUBO матрице које би потенцијално одговарале $K_4$ MaxCut проблему.
...
по енергији~\cite{kirkpatrick1983}. Самплер је квантно-инспирисан: не захтева стварни
квантни хардвер, али прати исту конвенцију интерфејса као и
правилни D-Wave анилер~\cite{johnson2011}, тако да је прелаз на хардвер тривијалан
...
# Simetricna QUBO matrica za MaxCut na K4
Q = {
    (0,0): -3, (0,1):  1, (0,2):  1, (0,3):  1,
    (1,0):  1, (1,1): -3, (1,2):  1, (1,3):  1,
    (2,0):  1, (2,1):  1, (2,2): -3, (2,3):  1,
    (3,0):  1, (3,1):  1, (3,2):  1, (3,3): -3,
}
...
Такође треба обратити пажњну да постоји више оптималиних решења (нпр.\ $(0,0,1,1)$ и $(1,1,0,0)$), па се може добити било које од њих.
...
предност тунелирања на хардверском нивоу~\cite{mcgeoch2014}.
\end{remark}
